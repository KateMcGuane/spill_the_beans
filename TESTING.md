...
| 5 | As per bug #4, many of the CSS features were not displaying as intended, nor were they the same as what was displaying when running the site from the server <br>  | - I deducted that the issue most likely was with one of the dependencies, and needed to ensure they were being implemented correctly <br> - As it could be a number of items causing the bug, I ran the settings.py through ChatGPT to confirm as much <br> | - Change STATIC_URL from <br> ```'static/'``` to ```'/static/'``` <br> This rendered no significant changes <br> - I discovered that the style.css from both the static directory and staticfiles directory were different <br> The staticfiles directory had not been updated with the changes made to static <br> - Ran ```python manage.py collectstatic
